...
selectWood = wrongpiecefont.render("Select a Wood piece", True, color)
...
#receviving data from server
def recevive_data():
    while True:
            global  server_pos , playerT
            data = sock.recv(1024).decode() #1024 is the byte
            data= data.split('-')
            server_pos = int(data[0])
            playerT= int(data[1])
            print(server_pos, playerT)
            ...
            screen.blit(stone, (tileGrid[server_pos].getxcoord() + 37, tileGrid[server_pos].getycoord() + 37))
            ...
            tileGrid[server_pos].drawiconrect()
            # set the player detection for player 1 to check for three in a row
            tileGrid[server_pos].setWhatPlayer(1)
                ...
                if event.type == pygame.MOUSEBUTTONUP:
                    ...
                    for i in range(0, 9):
                        ...
                        if pygame.Rect.collidepoint(tileGrid[i].getrectangle(), pygame.mouse.get_pos()):
                            ...
                            # send i over conncetion when tile is not occupied.
                            #if tileGrid[i].isoccupied() is None and :
                             #   pos = i
                              #  send_data = '{}'.format(pos).encode()  # encoing the i
                               # sock.send(send_data)  # sending i
                            if tileGrid[i].isoccupied() is None and playerTurn < 6:
                                #test
                                    pos = i
                                    send_data = '{}-{}'.format(pos,playerTurn).encode()  # encoing the i
                                    sock.send(send_data)  # sending i
                                ...
                                """
                                    ...
                                    # Display player turn at the butttom of the game board
                                    pygame.draw.rect(screen, p_color, [0, 700, 800, 60])
                        ...
                        # All pieces have been placed and it's now time to move them if no winner was found
                                ...
                                # If it's wood's turn
                                    ...
                                    # Give message if player tries clicking a stone icon during wood's turn
                                        ...
                                        pygame.draw.rect(screen, p_color, [0, 700, 800, 60])
                                        screen.blit(selectWood, (300, 700))
